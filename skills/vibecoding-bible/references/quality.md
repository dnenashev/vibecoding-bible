...
### Red для visual и вероятностного поведения

Red — это подтверждённое failing evidence, а не обязательно unit-тест. Для визуального,
вероятностного и недетерминированного поведения роль Red выполняет воспроизводимый probe:
зафиксированный сценарий, зафиксированный вход, заранее объявленный критерий и
повторяемый способ его получить.

Требования те же, что к обычному Red: он должен падать на baseline по ожидаемой причине,
не зависеть от случайного порядка и не подгоняться под фактический результат. Если
критерий нельзя объявить заранее, это не Red, а наблюдение — так его и называть.
...
### Человеческое ревью как канал evidence

Ревью — самостоятельный канал evidence наравне с тестами, но доказывает другое.

Ревью подтверждает: понятность и сопровождаемость решения, соответствие намерению и
контракту, замеченные пропущенные случаи, риск для соседних частей системы, уместность
выбранной абстракции.

Ревью не подтверждает: что код работает, что он не ломает существующее поведение, что
производительность приемлема, что интеграция жива. Это доказывают исполненные проверки.
«Отревьюено» не является заменой required evidence и не закрывает release gate само по себе.

Условия, при которых ревью считается evidence:

- ревьюер не является автором изменения;
- ревьюер видел diff и умеет запустить систему;
- замечания и их разрешение сохранены и связаны с exact candidate;
- при risk `CRITICAL` ревью consequential изменения выполняет независимый человек, а не
  тот же агент или его субагент.

Агент, написавший изменение, не подтверждает собственное изменение. Согласие пользователя
с объяснением агента не является ревью кода.
