...
Что требует проверки. Помечать load-bearing, если неверность меняет план.

## Refuted
Во что верили, что это убило и что верно теперь.
...
Для небольшого change хранить capsule в рабочем reasoning, а не создавать файл. Создавать artifact, если работа длинная, передаётся между agents или должна пережить сессию; durable-форма — [`belief-notes.md`](../assets/templates/belief-notes.md).

Держать capsule в пределах одной страницы. Лимит является механизмом: наблюдения конкурируют за место, поэтому сырые факты сжимаются в правила. Капсула без лимита растёт и перестаёт переживать compaction.

Раздел `Refuted` вести с самого начала. Отброшенное убеждение без записи возвращается: та же ошибка совершается второй раз, и второй раз она дороже.
...
### Проба, затем последовательность

Пока механика не подтверждена, менять одну переменную за probe. После подтверждения не платить один цикл за шаг: собрать последовательность, где **каждый шаг несёт своё ожидание, а исполнение останавливается на первом расхождении**. Неверная теория тогда стоит один шаг, а не весь план.

Порог «механика подтверждена» выводить из наблюдений этой задачи, а не из фиксированного числа проб. Не собирать в последовательность исследование: батчить можно только то, чей результат предсказуем заранее.

Это же правило применимо к миграциям, релизным спискам и любой длинной цепочке шагов — см. [`production.md`](production.md).
