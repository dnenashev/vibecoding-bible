...
### Подниматься на ступень по необходимости

Начинать с самого простого представления задачи, которое отвечает на вопросы: прозы и чтения кода, затем вычислений над собранными данными, затем исполняемой модели поведения. Подниматься на следующую ступень только когда текущая перестала отвечать, и записывать, **что именно** она перестала объяснять.

Ступень, выбранная заранее «на всякий случай», обычно не окупается и почти всегда переживает свою полезность.
